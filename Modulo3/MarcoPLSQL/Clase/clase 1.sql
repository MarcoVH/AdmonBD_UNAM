...
OPEN esclavos;
 ...
 dbms_output.put_line('la lectura del cursor es:'||nombre||'
'||apat||' '||amat);
 FETCH esclavos INTO nombre, apat,amat;
 --FETCH esclavos INTO nombre, apat,amat;
 END LOOP;
 ...
 is
 	select nom_emp, pat_emp, mat_emp
 	from empleado;
 vr_empleados c_empleados%ROWTYPE;
 BEGIN
 OPEN c_empleados;
 LOOP
 FETCH c_empleados INTO vr_empleados;
 exit when c_empleados%NOTFOUND;
 dbms_output.put_line('la lectura del cursor es:'||
vr_empleados.nom_emp||' '||vr_empleados.pat_emp||' '||
vr_empleados.mat_emp);
 END LOOP;
 CLOSE c_empleados;
 END;
 /