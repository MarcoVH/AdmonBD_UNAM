...
and to_number(to_char(fecha_nac_emp, 'fmDD'))>=22
order by fecha_nac_emp, pat_emp desc, mat_emp desc, nom_emp desc;

#2
select id_factura, fecha_factura, id_cliente, monto_factura,
rpad(to_char(iva_factura,'FM$99,999.00'),10,' ') as iva,
rpad(to_char(total_factura,'FM$99,999.00'),10,' ') as total
from udiplo.factura
...
next_day(last_day(fecha_factura)+15,1) as revision
from udiplo.factura
where mod(to_number(to_char(fecha_factura,'fmDD')),2)=0;
...
spool 'C:\Users\diplomado\Desktop\MarcoSQL\Pregunta8.txt'
/
spool off
